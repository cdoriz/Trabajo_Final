...
# Cargar estadísticas calculadas
stats_genero <- read_csv(here("output", "table", "04_stats_purchase_genero.csv"))
stats_edad <- read_csv(here("output", "table", "04_stats_purchase_edad.csv"))
stats_ciudad <- read_csv(here("output", "table", "04_stats_purchase_ciudad.csv"))
stats_product_category_1 <- read_csv(here("output", "table", "04_stats_purchase_product_category_1.csv"))
...
# Paleta de colores
color_masculino   <- "#084594"   # Azul oscuro
color_femenino <- "#f4b6c2"     # Rosa pastel
color_destacado   <- "#4292c6"   # Azul claro
color_secundario  <- "#c6dbef"   # Celeste claro
color_acento      <- "#6baed6"   # Celeste oscuro
color = "#D5A6E8"
...
ggsave(here("output", "figures", "05_cap6_interaccion_edad_genero.png"), 
       p5, width = 13, height = 8, dpi = 320, bg = "white")


# MENSAJE FINAL

cat("📊 GRÁFICOS GUARDADOS EN: output/figures/\n\n")

cat("🎯 INSIGHTS CLAVE:\n")
cat("   • Brecha de género: +", diferencia_genero$dif_pct, "% ($", 
    comma(round(diferencia_genero$dif_absoluta, 0)), ")\n", sep = "")
cat("   • Edad con mayor gasto: ", edad_mayor_gasto$Age, " años ($", 
    comma(round(edad_mayor_gasto$media, 0)), ")\n", sep = "")
cat("   • Ciudad líder: ", ciudad_mayor$City_Category, " ($", 
    comma(round(ciudad_mayor$media, 0)), ")\n", sep = "")
cat("   • Categoría premium: ", producto_estrella$Product_Category_1, " ($", 
    comma(round(producto_estrella$media, 0)), ")\n\n", sep = "")

cat("✅ CONCLUSIÓN: H0 RECHAZADA\n")
cat("   El consumo varía significativamente por todas las variables analizadas.\n\n")

cat("📈 ARCHIVOS GENERADOS:\n")
cat("   1. Historia completa (combinado)\n")
cat("   2-7. Capítulos individuales\n")
cat("   Resolución: 320 DPI (print-ready)\n\n")