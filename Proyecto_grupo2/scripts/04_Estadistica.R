...
write_csv(normalidad_df, here("output", "table", "04_normalidad_purchase.csv"))
print(normalidad_df)
...
write_csv(levene_results, here("output", "table", "04_levene_test_results.csv"))
...
write_csv(tests_no_parametricos, here("output", "table", "04_tests_no_parametricos.csv"))
...
write_csv(coefs_base_df, here("output","table","04_coeficientes_modelo_base.csv"))

# Métricas de bondad de ajuste - MODELO BASE
glance_base <- glance(modelo_base) %>%
  select(r.squared, adj.r.squared, sigma, AIC, BIC, p.value) %>%
  mutate(modelo = "Base (sin Product_Category_1)")

write_csv(glance_base, here("output", "table", "04_bondad_ajuste_modelo_base.csv"))
...
write_csv(coefs_completo_df, here("output", "table", "04_coeficientes_modelo_completo.csv"))

# Métricas de bondad de ajuste - MODELO COMPLETO
glance_completo <- glance(modelo_completo) %>%
  select(r.squared, adj.r.squared, sigma, AIC, BIC, p.value) %>%
  mutate(modelo = "Completo (con Product_Category_1)")

write_csv(glance_completo, here("output", "table", "04_bondad_ajuste_modelo_completo.csv"))

# COMPARACIÓN DE MODELOS
comparacion_modelos <- bind_rows(glance_base, glance_completo) %>%
  mutate(across(where(is.numeric), ~round(., 6)))

write_csv(comparacion_modelos, here("output", "table", "04_comparacion_modelos.csv"))
...
if(is.matrix(vif_vals)) {
  # Para variables categóricas, usar GVIF^(1/(2*Df))
  vif_df <- tibble(
...
write_csv(vif_df, here("output", "table", "04_vif_modelo_completo.csv"))
...
write_csv(bp_df, here("output", "table", "04_breusch_pagan_test.csv"))
...
if(bp_test$p.value < 0.05) {
  ...
  write_csv(coefs_robust_df, here("output", "table", "04_coefs_robustos.csv"))
}

# ANOVA Type III (adecuado para modelos con dummies y sin homocedasticidad)
anova_type3 <- car::Anova(modelo_completo, type = 3)

anova_type3_df <- broom::tidy(anova_type3) %>%
  mutate(significativo = ifelse(p.value < 0.05, "Sí", "No"))

write_csv(anova_type3_df, here("output", "table", "04_anova_type3.csv"))
...
write_csv(anova_robusta_df, here("output", "table", "04_anova_robusta_hc3.csv"))
...
write_csv(chi_df, here("output", "table", "04_chi_cuadrado.csv"))
...
write_csv(resid_df, here("output", "table", "04_residuos.csv"))
...
write_csv(stats_genero, here("output", "table", "04_stats_purchase_genero.csv"))
write_csv(stats_edad, here("output", "table", "04_stats_purchase_edad.csv"))
write_csv(stats_ciudad, here("output", "table", "04_stats_purchase_ciudad.csv"))
write_csv(stats_product_category_1, here("output", "table", "04_stats_purchase_product_category_1.csv"))

# RESUMEN FINAL COMPARATIVO
cat("\n=== RESUMEN COMPARATIVO DE MODELOS ===\n")
print(comparacion_modelos)
cat("\nMejora en R²:", 
    round((glance_completo$r.squared - glance_base$r.squared) * 100, 4), "%\n")